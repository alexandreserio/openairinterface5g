...
int nrue_ru_adjust_rx_gain(PHY_VARS_NR_UE *UE, int gain_change)
{
  ...
  */ 
  if ((cfg->rx_gain[0] == MAX_RF_GAIN) && (!UE->is_synchronized)){
    LOG_E(HW, "Max usrp rx gain reached! Reseting rx_gain to MIN_RF_GAIN (%d)...\n", MIN_RF_GAIN);
    cfg->rx_gain[0] = MIN_RF_GAIN;
  }
  else if (cfg->rx_gain[0] + gain_change > MAX_RF_GAIN){
    cfg->rx_gain[0] = MAX_RF_GAIN;
  }
  else if (cfg->rx_gain[0] + gain_change < MIN_RF_GAIN){
    cfg->rx_gain[0] = MIN_RF_GAIN;
  }
  else {
    cfg->rx_gain[0] += gain_change;
  }
  UE->rx_total_gain_dB = cfg->rx_gain[0];
  // Set new RX gain.
  //LOG_ME(PHY, "cfg->rx_gain: %.2f\n", cfg->rx_gain[0]); //ALEX debug
